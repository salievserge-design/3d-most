...
# Балка Б.140.123 (двутавр, серия 3.503.1-81): h=1.23 м, b_верх=1.40 м
# Контейнер сечения: верхняя полка 0.18, стенка 0.26, нижняя полка 0.18 (0.06..0.24),
# "обувь" (выступающая снизу часть) 0.36×0.06 — опирается на ростверк.
def beam_profile_mm():
    # Размер сечения модели (1:100), мм: b=14.0, h=12.3
    return [
        (-7.0, 12.3), (7.0, 12.3),     # верхняя полка (под настилку)
        (7.0, 10.5), (1.3, 10.5),      # нижний торец верхней полки -> стенка
        (1.3, 2.4), (3.5, 2.4),        # стенка -> нижняя полка
        (3.5, 0.6), (1.8, 0.6),        # нижняя полка -> "обувь"
        (1.8, 0.0), (-1.8, 0.0),       # "обувь" (выступает снизу)
        (-1.8, 0.6), (-3.5, 0.6),
        (-3.5, 2.4), (-1.3, 2.4),
        (-1.3, 10.5), (-7.0, 10.5),
    ]
...
def make_beam(L_m):
    m = Mesh()
    prof = [(y + 7.0, z) for (y, z) in beam_profile_mm()]
    prof = list(reversed(prof))  # CW -> CCW для правила в extrude_profile
    m.extrude_profile(prof, 0.0, L_m * SCALE)
    return m

# Раскладка балок в поперечном сечении: 5 шт. × 1.40 м, зазор 0.10 м (м.м. настилки)
BEAM_Y = (-3.0, -1.5, 0.0, 1.5, 3.0)   # центры, м
BEAM_Z = 5.0                            # низ балки = верх ростверка/устоя
...
# Настилка (участки омоноличивания): лежит на верхних полках балок
DECK = dict(wide=8.0, th=0.35, z=6.23)
